...
- **SPCX is a pre-IPO synthetic of a private company, priced from a single third-party feed.** SPCX tracks a SpaceX synthetic from a third-party Hyperliquid feed. It is **not** SpaceX stock and confers no ownership or equity claim. SpaceX is private, so there is no public equity market to anchor the price; SPCX has no Pyth feed and is the only market sourced solely from Hyperliquid. If that feed is wrong, delayed, or unavailable, there is no second on-chain source — the market freezes or can be deactivated. Treat it as a speculative synthetic. See [Markets](/markets).
