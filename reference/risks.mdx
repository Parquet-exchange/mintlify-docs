...
- **SPCX is a synthetic priced from a single third-party feed.** SPCX tracks a SpaceX synthetic from a third-party reference feed — it tracks that off-exchange synthetic, not the underlying SpaceX shares. It is **not** SpaceX stock and confers no ownership or equity claim. SPCX has no independent on-chain oracle feed and, like several other markets, is sourced solely from a third-party reference feed. If that feed is wrong, delayed, or unavailable, there is no second on-chain source — the market freezes or can be deactivated. Treat it as a speculative synthetic. See [Markets](/markets).
