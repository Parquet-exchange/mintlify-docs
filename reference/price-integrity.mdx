...
Parquet is **oracle-priced and pool-backed** — there is no order book. Every position opens, closes, and liquidates at the oracle mark against a per-market USDC pool. That makes the integrity of the price the single most important safety property on the venue, so it is defended in layers: bad or stale data is filtered **off-chain before it is ever posted**, and then checked **again on-chain before any trade can execute against it**.
...
- **The regular-hours on-chain mark is a decentralized aggregate, not a feed we run.** During RTH the primary on-chain price is sourced from **Pyth Network**, which aggregates many independent publishers. Moving it means moving that aggregate — not spoofing a single venue or a single relay we control.
