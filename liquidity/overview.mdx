...
description: How Parquet's shared USDC liquidity pool works — depositing, withdrawing, earning fees, and understanding risks.
---
...
Every Parquet market is backed by a single, shared USDC liquidity pool. When you deposit USDC, you receive LP tokens proportional to your share of the pool's total value. The pool is the counterparty to every trade on the venue: when traders are net profitable it pays out; when they lose, it earns.

Because one pool stands behind every market, providing liquidity gives you diversified exposure across the whole venue rather than a single ticker — a run of profitable AAPL traders is netted against trader losses elsewhere in the same pool.

## Depositing

Deposit USDC into the pool to mint LP tokens. The number of LP tokens you receive is based on the current pool value at the time of deposit.
...
LPs earn through the fee accrual mechanism. On every trade in any market the pool backs, the LP share of the position fee is added directly to the pool's USDC value. This increases the value of each LP token over time.
...
- **Correlated drawdowns:** The shared pool is counterparty to every market it backs, so it nets away single-ticker idiosyncratic risk — but not market-wide moves. A broad rally that's profitable for net-long traders across many names at once draws the whole pool down together.
